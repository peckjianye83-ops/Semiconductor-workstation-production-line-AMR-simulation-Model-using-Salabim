...
def run_simulation():
    ...
    NUM_AREAS = 10
    TOTAL_MACHINES = 40
    TOTAL_AMRS = 10  # <--- Change this to any number you want!
    # -----------------------------------------
    ...
    # =========================================================================
    # 2. Graph Architecture & Dynamic Parking Calculation
    # =========================================================================
    global graph
    graph = {}
    def add_edge(n1, n2):
    ...
    # --- DYNAMIC AMR PARKING SLOTS GENERATION ---
    num_top_parking = math.ceil(TOTAL_AMRS / 2)
    num_bot_parking = TOTAL_AMRS - num_top_parking
    
    park_x_positions_top = [200 + i * 50 for i in range(num_top_parking)]
    park_x_positions_bottom = [200 + i * 50 for i in range(num_bot_parking)]
    
    # Calculate furthest bounds for UI rendering later
    highest_px = max([480] + park_x_positions_top + park_x_positions_bottom)
    max_station_x = highest_px + 50
    ...
    # Instantiate AMRs and Distribute among areas dynamically
    park_slots = []
    ...
    # Dynamically expand return lane lines to meet new parking slot widths
    sim.AnimateLine([450, 40, highest_px, 40], linewidth=4, linecolor="limegreen")
    sim.AnimateLine([450, 915, highest_px, 915], linewidth=4, linecolor="limegreen")
    ...
    # Scaled Background UIs based on new AMR counts
    sim.AnimateRectangle(spec=(150, 0, max_station_x, 50), fillcolor="skyblue")
    sim.AnimateText("AMR STATION 1", x=(150 + max_station_x)/2, y=40, textcolor="black", text_anchor="c", fontsize=12, font="bold")
    for px in park_x_positions_top: sim.AnimateRectangle(spec=(px - 11, 1, px + 11, 23), fillcolor="salmon")
    
    sim.AnimateRectangle(spec=(150, 900, max_station_x, 950), fillcolor="skyblue")
    sim.AnimateText("AMR STATION 2", x=(150 + max_station_x)/2, y=920, textcolor="black", text_anchor="c", fontsize=12, font="bold")
